...
export class CachedFileSystem {
  ...
  private enforceCacheSize(cache: Map<string, { timestamp: number }>): void {
    if (cache.size > this.maxCacheSize) {
      ...
      const toRemove = entries.slice(0, Math.floor(this.maxCacheSize / 10)); // Remove 10% of entries when limit is reached
  ...
  async stat(filePath: string): Promise<Stats | null> {
    ...
    if (this.fileStatCache.size > 0 && this.fileStatCache.size % 100 === 0) {
  ...
  async readFile(
    ...
    if (
      ...
      this.fileContentCache.size % 100 === 0
    ) {
  ...
  async readDir(dirPath: string): Promise<string[]> {
    ...
    // Clean expired entries periodically
    if (this.directoryCache.size > 0 && this.directoryCache.size % 100 === 0) {
